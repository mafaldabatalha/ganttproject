...
public class HumanResource implements CustomPropertyHolder {
    ...
    public int overlappingDates(GanttCalendar startDate, GanttCalendar endDate) {
        ...
        while (it.hasNext()) {
            ...
            if (areDatesInConflict(startDate, endDate, taskStartDate, taskEndDate))
                overloadedTasks++;
        }
    ...
    /**
     * Metodo que verifica se, para a tarefa que se esta a atribuir a um recurso, há conflito com as suas férias.
     *
     * @param startDate - data de inicio
     * @param endDate   - data de fim
     * @return - retorna null no caso de não existir nenhum conflito com as datas, ou retorna a data onde há o conflito.
     */
    public GanttDaysOff overlappingHolidays(GanttCalendar startDate, GanttCalendar endDate) {
        Object[] daysOff = myDaysOffList.toArray();

        for (int i = 0; i < daysOff.length; i++) {
           if (daysOff[i] instanceof GanttDaysOff) {
                GanttDaysOff holidays = (GanttDaysOff) daysOff[i];
                GanttCalendar dayOffStartDate = holidays.getStart();
                GanttCalendar dayOffEndDate = holidays.getFinish();
                if (areDatesInConflict(startDate, endDate, dayOffStartDate, dayOffEndDate))
                    return holidays;
            }
        }
        return null;
    ...
    /**
     * Metodo auxiliar ao metodo overlappingDates. Testa se as datas de uma tareda sao
     * compativeis com as datas de outra tareda ja existente.
     *
     * @param startDate     - data de inicio
     * @param endDate       - data do fim
     * @param taskStartDate - data de inicio da tarefa a ser comparada
     * @param taskEndDate   - data de fim da tarefa a ser comparada
    ...
    /**
     * Function that verifies if the task is already in the humanResource system.
     *
     * @param task - task to verify
     * @return - returns <code> true </code>, if the task is already associated with the resource, otherwise returns <code> false </code>.
     */
